...
plugin {
  tx_t3vproducts {
    persistence {
      ...
      # cat=plugin/tx_t3vproducts/persistence; type=int+; label=The PID of the default storage
      storagePid = 0

      # cat=plugin/tx_t3vproducts/persistence; type=boolean; label=Updates the reference index to ensure data integrity
      updateReferenceIndex = 1
    }

    settings {
      extbase {
        # cat=plugin/tx_t3vproducts/settings/extbase; type=string; label=The controller extension name
        controllerExtensionName = T3vProducts
      }
    }
